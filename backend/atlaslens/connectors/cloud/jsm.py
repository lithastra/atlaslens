...
class JsmCloudConnector:
    ...
    async def fetch_audit(self, cursor: Cursor) -> list[RawEvent]:
        ...
        while True:
            ...
            data = resp.json()
            records: list[dict] = data.get("records", [])  # type: ignore[type-arg]
            ...
            for record in records:
                ...
                if event_source not in ("jsm", "jira service management"):
                    continue

                events.append(
                    RawEvent(
                        source_id=f"jsm-{record['id']}",
                        occurred_at=_parse_date(record["created"]),
        ...
        logger.info("jsm cloud audit: fetched %d records", len(events))
        return events
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                if resp.status_code == 429:
                    ...
                    logger.warning(
                        "jsm: rate-limited, retrying in %.1fs", wait
                    )
